...
### Sample exclusions
Each subject should have exactly one recorded choice (`listing_chosen`) per
weekend, across all 12 weekends (3 cities x 4 weekends). Subjects with fewer
than 12 recorded choices — e.g. a `reserve` event that never got tracked for
some weekend, or a session abandoned partway through — are dropped from the
analysis dataset before any hypothesis test below.
```{r}
...
```{r}
# Following Chiara's advice, we'll collapse this to the 
...
# TODO: IS THIS LOGGED?, also ways we deal with 0s, log 1+ x
...
```{r}
# TODO: UPDATE TO PREREGISTRATION 
...
# TODO : Make sure this doesn't involve loading
m_h231 <- feols(
...
# TODO: Print average preference consistency, at indv-time-city level
```
